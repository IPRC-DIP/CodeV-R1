        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_part in self.train_dataloader:
                ...
                if self.dapo_accelerate:
                    ...
                    if len(new_batch) < num_gen_remaining / min(self.select_ratio, self.select_ratio_part):
                        continue
                    else:
                        new_batch_flag = True

                    print(f'len(new_batch) is {len(new_batch)}')
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        if self.dapo_accelerate:
                            ...
                            if len(kept_prompt_uids) >= num_gen_remaining:
                                self.select_ratio = min(self.select_ratio, 1.0 * (self.config.data.train_batch_size - num_gen_remaining + len(kept_prompt_uids)) / dapo_raw_batch_size_accumulate)
                                self.select_ratio_part = 1.0
                                dapo_raw_batch_size_accumulate = 0
